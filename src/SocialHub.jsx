        ...
        <div style={{flex:1,overflowY:'auto',padding:'18px',background:C.bg}}>
          ...
          {mainTab==='wizard'&&(()=>{
            ...
            return(
              <div>
                <StepBar/>
                {wizardStep===1&&(<div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:12,padding:20}}><div style={{fontSize:14,fontWeight:700,color:C.text,marginBottom:3}}>✍️ Write your post</div><div style={{fontSize:12,color:C.muted,marginBottom:13}}>Paste AI-generated content or write from scratch.</div><label style={labelStyle}>Post content</label><textarea value={wizardContent} onChange={e=>setWizardContent(e.target.value)} rows={8} style={{...inputStyle,fontSize:14,lineHeight:1.6,marginBottom:4}} placeholder={"Share something that reflects your philosophy…

"Be Good. Do Good. Do Well.""}/><div style={{display:"flex",justifyContent:"space-between",marginTop:2,marginBottom:13}}><span style={{fontSize:11,color:C.muted}}>{wizardContent.length} characters</span>{wizardContent.length>300&&<span style={{fontSize:11,color:"#d97706",fontWeight:500}}>⚠ Over Bluesky limit</span>}</div><label style={labelStyle}>Image URL <span style={{color:"#bbb",fontWeight:400}}>(optional)</span></label><input value={wizardImage} onChange={e=>setWizardImage(e.target.value)} style={{...inputStyle,marginBottom:13}} placeholder="https://img.youtube.com/vi/VIDEO_ID/maxresdefault.jpg"/>{wizardImage&&<img src={wizardImage} alt="" style={{maxWidth:"100%",maxHeight:130,borderRadius:8,border:`1px solid ${C.border}`,objectFit:"cover",marginBottom:13}} onError={e=>{e.target.style.display="none";}}/>}<label style={labelStyle}>Quick fill from saved quotes</label><div style={{display:"flex",flexWrap:"wrap",gap:6}}>{Object.entries({integrity:"Integrity edge",reputation:"Reputation",shortcuts:"Shortcuts",nepal:"Nepal roots",ai:"AI & you",trust:"Trust"}).map(([k,v])=><button key={k} onClick={()=>setWizardContent(QUOTES[k]||"")} style={{padding:"4px 10px",fontSize:11,borderRadius:14,border:`1px solid ${C.border}`,background:"#f9f9f9",cursor:"pointer",color:C.muted}}>{v}</button>)}</div></div>)}
                {wizardStep===2&&<Step2/>}
                {wizardStep===3&&<Step3/>}
                {wizardStep===4&&<Step4/>}
                {wizardStep===5&&<Step5/>}
