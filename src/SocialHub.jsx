...
const parsedIdeas=JSON.parse(cleaned2);
  ...
  const MEDIA_STUDIO_API=import.meta.env.VITE_MEDIA_STUDIO_API||'https://strategic-honesty-media-studio-production.up.railway.app';function _postToQueueItem(post){const firstLine=(post.content||'').split('\n').find(l=>l.trim())||'Untitled';const title=firstLine.replace(/^#+\s*/,'').replace(/\*\*/g,'').slice(0,80);const typeMap={LinkedIn:'LinkedIn Post',TikTok:'TikTok Video','YouTube Short':'YouTube Short',Instagram:'Instagram Reel',Facebook:'Facebook Post','X/Twitter':'X/Twitter Post'};return{title,platform:post.platform||'Unknown',contentType:post.contentType||typeMap[post.platform]||'Text Post',prompt:post.imagePrompt||'',source:'scheduler'};}async function sendAllToMediaStudio(){const posts=queue||[];if(!posts.length){alert('No approved posts.');return;}try{const items=[];for(const post of posts){const item=_postToQueueItem(post);item.sourceText=post.content||post.text||post.imagePrompt||'';const pl=(post.platform||'').toLowerCase();const isVid=pl.includes('tiktok')||pl.includes('youtube')||pl.includes('instagram');if(isVid){try{const scenes=await generateScenes(item.sourceText,post.platform,item.contentType);item.sceneData=scenes;}catch(e){console.warn('Scene gen failed',e.message);}}items.push(item);}const res=await fetch(MEDIA_STUDIO_API+'/api/queue',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({items})});if(!res.ok)throw new Error('Server '+res.status);const d=await res.json();alert('Sent '+d.count+' post(s) to Media Studio!');}catch(e){alert('Failed: '+e.message);}}
